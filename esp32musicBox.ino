...
//A rough and ready tune playing engine.
//Licence GPLv3
...
//============================================
#define DAC1 25 //Uses DAC on pin 25
...
int indexer = 0; //Note count 
//(would have used "index" but that seems to be reserved as a pointer for ESP32 core)
...
3.88169, 4.11251, 4.35705, 4.61614, 4.89063, 5.18144, 5.48954, 5.81597, 6.16181, 0}; 
//Increase per sample (governs note frequency) last value (0) gives rest
...
//Array containing tune. Trios of values give note number, duration and slur, 1 for slurred notes.
...
void loop() 
{
  ...
  if (currentMicros - previousMicros >= UperiodConst) 
//Loop to build sawtooth increasing output by "sampleVal" every sample and reset to zero when output is max 
  {
    previousMicros = currentMicros; //Resets micros for next sample
    sampleVal = pgm_read_float(&sample[pgm_read_byte(&swan[indexer])]); 
    //reads note from swan to index sample array from PROGMEM to produce correct note.
    dacWrite(DAC1, int(value)); //Writes samples to DAC as ints
 ...
 if (currentMillis - previousMillis >= tempo * pgm_read_byte(&swan[indexer + 1]) && value <= 1) 
 ...
 {
  dacWrite(DAC1, 0); //Turns off output for between notes
  if(pgm_read_byte(&swan[indexer + 2]) == 0) //Tests slur parameter from swan
  { 
  delay(40); //Normal note off duration (between notes)
  }
  if(pgm_read_byte(&swan[indexer + 2]) == 1)
  { 
  delay(0); //Slur note off duration (between notes) 
  }
  indexer = indexer + 3; //reads next note from tune array.
  
  previousMillis = currentMillis; //reset timer for next note
  if(indexer >= sizeof(swan)) //Stops at end of array and returns to beginning
  {
    indexer = 0;
  }
...
//const byte wewish[157] PROGMEM = {100, 25, 4, 0, /*BAR*/ 30, 4, 0, 30, 2, 1, 32, 2, 1, 30, 2, 1, 29, 2, 1,/*BAR*/ 27, 4, 0, 27, 4, 0,/*25*/
//27, 4, 0, /*BAR*/ 32, 4, 0, 32, 2, 1, 34, 2, 1, 32, 2, 1, 30, 2, 1, /*BAR*/29, 4, 0, 25, 4, 0, 25, 4, 0, /*BAR*/ /*27*/
//34, 4, 0, 34, 2, 1, 35, 2, 1, 34, 2, 1, 32, 2, 1, /*BAR*/30, 4, 0, 27, 4, 0, 25, 2, 0, 25, 2, 0, /*BAR*/27, 4, 0, 32, 4, 0, /*33*/
//29, 4, 0,/*BAR*/ 30, 8, 0, 25, 4, 0, /*BAR*/30, 4, 0, 30, 4, 0, 30, 4, 0, /*BAR*/29, 8, 0, 29, 4,  0,/*BAR*/ /*24*/
//30, 4, 0, 29, 4, 0, 27, 4, 0, /*BAR*/ 25, 8, 0, 25, 4,  0,/*BAR*/34, 4, 0, 32, 4, 0, 30, 4, 0, /*BAR*/37, 4, 0, 25, 4, 0, /*30*/
//25, 2, 0, 25, 2, 0, /*BAR*/27, 4, 0, 32, 4, 0, 29, 4, 0, /*BAR*/ 30, 8, 0}; /*18*/
//Array containing tune. Tempo (first element) along with note number and duration + slur, 1 for slurred notes.
